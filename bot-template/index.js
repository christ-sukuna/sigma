...
async function startGifted() {
    try {
        ...
        // ── VPS pairing code support ──────────────────────────────────────────
        const PAIRING_NUMBER = (process.env.PAIRING_NUMBER || '').replace(/\D/g, '');
        const SIGMA_CB = process.env.SIGMA_CALLBACK_URL;

        if (PAIRING_NUMBER && !state.creds?.registered) {
            setTimeout(async () => {
                try {
                    const code = await Gifted.requestPairingCode(PAIRING_NUMBER);
                    const formatted = code?.match(/.{1,4}/g)?.join('-') || code;
                    console.log(`[SIGMA] Pairing code: ${formatted}`);
                    if (SIGMA_CB) {
                        axios.post(SIGMA_CB, { type: 'paircode', code: formatted }).catch(() => {});
                    }
                } catch (e) {
                    console.error('[SIGMA] Pairing code error:', e.message);
                }
            }, 3000);
        }
        // ─────────────────────────────────────────────────────────────────────
        ...
        setupConnectionHandler(Gifted, sessionDir, startGifted, {
            onOpen: async (Gifted) => {
                // Notify SIGMA MDX platform if deployed via VPS
                if (process.env.SIGMA_CALLBACK_URL) {
                    axios.post(process.env.SIGMA_CALLBACK_URL, { type: 'connected' }).catch(() => {});
                }
