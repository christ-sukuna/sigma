...
async function loadSession() {
    try {
        ...
        const VALID_PREFIXES = ['SIGMA-MDX', 'Gifted'];
        if (!VALID_PREFIXES.includes(headerCheck) || !b64Check) {
            throw new Error("❌ Invalid session format. Expected 'SIGMA-MDX~.....'");
        }
        ...
        if (!b64Check.startsWith('H4sI')) {
            ...
            const isValidFetched = VALID_PREFIXES.some(p => fetched.startsWith(p + '~H4sI'));
            if (!isValidFetched) {
        ...
        if (!VALID_PREFIXES.includes(header) || !b64data) {
            throw new Error("❌ Invalid session format. Expected 'SIGMA-MDX~.....'");
        }
